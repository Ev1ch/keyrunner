export const texts = [
  'Text for typing #1. Text for typing #1. Text for typing #1. Text for typing #1. Text for typing #1.',
  'Text for typing #2. Text for typing #2. Text for typing #2. Text for typing #2. Text for typing #2.',
  'Text for typing #3. Text for typing #3. Text for typing #3. Text for typing #3. Text for typing #3.',
  'Text for typing #4. Text for typing #4. Text for typing #4. Text for typing #4. Text for typing #4.',
  'Text for typing #5',
  'Text for typing #6',
  'Text for typing #7',
];

export const jokes = [
  'A man got hit in the head with a can of Coke. Thank goodness it was a soft drink.',
  'What do you call cheese that isn’t yours? Nacho Cheese.',
  'My girlfriend and I often laugh about how competitive we are. But I laugh more.',
  'What do you call a fish with no eye? Fssshh.',
  'What’s white and can’t climb trees? A fridge.',
  'A communist joke isn’t funny…unless everyone gets it.',
  'What’s the stupidest animal in the jungle? A polar bear.',
  'Don’t you hate it when someone answers their own questions? I do.',
  'What do you call a fake noodle? An Impasta.',
  'The shovel was a ground-breaking invention.',
];

export const facts = [
  "Glaciers and ice sheets hold about 69 percent of the world's freshwater.",
  'The fastest gust of wind ever recorded on Earth was 253 miles per hour.',
  'The best place in the world to see rainbows is in Hawaii.',
  'There are fossilized plants in Greenland under 1.4 km of ice.',
  'Whale songs can be used to map out the ocean floor.',
  'New creatures have been found in deep-sea volcanoes.',
  'Mount Everest is bigger now than the last time it was measured.',
  'Climate change is causing flowers to change color.',
  'Dentistry is the oldest profession in the world.',
];

export const cars = [
  'Mercedes',
  'Toyota',
  'Lamborghini',
  'Porsche',
  'BMW',
  'Lada',
  'Nissan',
  'Bentley',
  'Volkswagen',
  'Mitsubishi',
];

export const salutPhrase =
  'Hello! My name is Olexander and I am the commentator of your game. Now your have to wait a bit and we will start!';

export const startPhrase = 'Here we go!';

export const goodbyePhrase = 'It was a cool game. I was really enjoing it.';
